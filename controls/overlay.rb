...
include_controls 'red-hat-enterprise-linux-6-stig-baseline' do
  ...
  control 'V-38477' do
    ...
    desc 'fix', 'To specify password minimum age for new accounts, edit the file "/etc/login.defs" and add or correct the following line, replacing [DAYS] appropriately: 
		...
		A value of 1 day is considered sufficient for many environments. The CMS requirement is 1.'
  end
  ...
  control 'V-38479' do
    ...
    desc 'fix', 'To specify password maximum age for new accounts, edit the file 
		"/etc/login.defs" and add or correct the following line, replacing [DAYS] appropriately: 
		PASS_MAX_DAYS [DAYS]
  ...
  control 'V-38482' do
    desc 'check', 'To check how many digits are required in a password, run the following command: 
		...
		Note: The "dcredit" parameter (as a negative number) will indicate how many digits are required. CMS requires at least one digit in a password. This would appear as "dcredit=-1".
  ...
  control 'V-38501' do
    desc 'title', 'The system must disable accounts after excessive login failures within a 120-minute interval.'
    desc 'check', 'To ensure the failed password attempt policy is configured correctly, run the following command:

		$ grep pam_faillock /etc/pam.d/system-auth /etc/pam.d/password-auth

		For each file, the output should show "fail_interval=<interval-in-seconds>" where "interval-in-seconds" is 7200 (120 minutes) or greater. 
		
		If that is not the case, this is a finding.'
    desc 'fix', 'Utilizing "pam_faillock.so", the "fail_interval" directive configures the system to lock out accounts after a number of incorrect logon attempts. 
  ...
  control 'V-38569' do
    desc 'title', 'The system must require passwords to contain at least one uppercase alphabetic characters.'
    desc 'check', 'To check how many uppercase characters are required in a password, run the following command: 

		$ grep pam_cracklib /etc/pam.d/system-auth /etc/pam.d/password-auth

		Note: The "ucredit" parameter (as a negative number) will indicate how many uppercase characters are required. CMS requires at least one uppercase character in a password. This would appear as "ucredit=-1". 
  ...
  control 'V-38570' do
    desc 'tite', 'The system must require passwords to contain at least one special character.'
    desc 'check', 'To check how many special characters are required in a password, run the following command: 

    $ grep pam_cracklib /etc/pam.d/system-auth /etc/pam.d/password-auth

    Note: The "ocredit" parameter (as a negative number) will indicate how many special characters are required. CMS requires at least one special character in a password. This would appear as "ocredit=-1".

    If ocredit is not found or not set to the required value, this is a finding.'
    desc 'fix', 'The pam_cracklib module\'s "ocredit=" parameter controls requirements for usage of special (or "other") characters in a password. When set to a negative number, any password will be requied to contain that many special characters. When set to a positive number, pam_cracklib will grant +1 additional length credit for each special character. 
  ...
  control 'V-38571' do
    desc 'title', 'The system must require passwords to contain at least one lower-case alphabetic character.'
    desc 'check', 'To check how many lower-case characters are required in a password, run the following command: 
    
    $ grep pam_cracklib /etc/pam.d/system-auth /etc/pam.d/password-auth

    Note: The "lcredit" parameter (as a negative number) will indicate how many lower-case characters are required. CMS requires at least one lower-case character in a password. This would appear as "lcredit=-1". 

    If lcredit is not found or not set to the required value, this is a finding.'
    desc 'fix', 'The pam_cracklib module\'s "lcredit=" parameter controls requirements for usage of lower-case letters in a password. When set to a negative number, any password will be required to contain that many lower-case characters. 
  ...
  control 'V-38572' do
    desc 'title', 'The system must require at least six characters be changed between the old and new passwords during a password change.'
    desc 'check', 'To check how many characters must differ during a password change, run the following command: 

    $ grep pam_cracklib /etc/pam.d/system-auth /etc/pam.d/password-auth

    Note: The "difok" parameter will indicate how many characters must differ. CMS requires that six characters differ during a password change. This would appear as "difok=6". 
    ...
    Edit /etc/pam.d/system-auth and /etc/pam.d/password-auth adding "difok=[NUM]" after pam_cracklib.so to require differing characters when changing passwords, substituting [NUM] appropriately. The CMS requirement is 6.'
  end

  control 'V-38573' do
    desc 'title', 'The system must disable accounts after five consecutive unsuccessful logon attempts.'
    desc 'check', 'To ensure the failed password attempt policy is configured correctly, run the following command: 
    # grep pam_faillock /etc/pam.d/system-auth /etc/pam.d/password-auth
  ...
  control 'V-38592' do
    title 'The system must require an account to be locked out for at least an hour for an account locked by excessive failed login attempts.'
  ...
  control 'V-38593' do
    desc 'title', 'The CMS login banner must be displayed immediately prior to, or as part of, console login prompts.'
    desc 'check', 'To check if the system login banner is compliant, run the following command: 
    $ cat /etc/issue

    Note: The full text banner must be implemented unless there are character limitations that prevent the display of the full CMS logon banner.
    If the required CMS logon banner is not displayed, this is a finding.'
    desc 'fix', 'To configure the system login banner: 

    Edit "/etc/issue". Replace the default text with a message compliant with the local site policy or a legal disclaimer. The CMS required text is either: 
  ...
  control 'V-38608' do
    ...
    The timeout [interval] is given in seconds. To have a timeout of 30 minutes, set [interval] to 1800. 
  ...
  control 'V-38621' do
    desc 'title', 'The system clock must be synchronized to an authoritative CMS time source.'
  end
  ...
  control 'V-38658' do
    ...
    If the line is commented out, the line does not contain "password required pam_pwhistory.so" or "password requisite pam_pwhistory.so", or the value for "remember" is less than ‚ 6, this is a finding.'
    desc 'fix', 'Do not allow users to reuse recent passwords. This can be accomplished by using the "remember" option for the "pam_pwhistory" PAM module. In the file "/etc/pam.d/system-auth" and /etc/pam.d/password-auth, append "remember=6" to the lines that refer to the "pam_pwhistory.so" module, as shown:
  ...
  control 'V-38685' do
    ...
    Verify each of these accounts has an expiration date set at 30 days from creation. If any temporary accounts have no expiration date set or do not expire within 30 days, this is a finding.'
    desc 'fix', 'In the event temporary accounts are required, configure the system to terminate them after 30 days. For every temporary account, run the following command to set an expiration date on it, substituting "[USER]" and "[YYYY-MM-DD]" appropriately: 
  ...
  control 'V-38689' do
    desc 'title', 'The CMS login banner must be displayed immediately prior to, or as part of, graphical desktop environment login prompts.'
    ...
    * By using this system, you understand and consent to the following:
    ...
    Therefore, you have no reasonable expectation of privacy regarding any communication or data transiting or stored on this system. At any time, and for any lawful Government purpose, the government may monitor, intercept, and search and seize any communication or data transiting or stored on this system.
    - Any communication or data transiting or stored on this system may be disclosed or used for any lawful Government purpose" 
    
    If the CMS required banner text does not appear in the schema, this is a finding.'
    desc 'fix', 'To set the text shown by the GNOME Display Manager in the login screen, run the following command: 
  ...
  control 'V-38690' do
    ...
    Verify each of these accounts has an expiration date set within 24 hours of being created. If any emergency accounts have no expiration date set or do not expire within a documented time frame, this is a finding.'
  end
  ...
  control 'V-38692' do
    ...
    A value of 60 is recommended. If a password is currently on the verge of expiration, then 60 days remain until the account is automatically disabled. However, if the password will not expire for another 60 days, then 120 days could elapse until the account would be automatically disabled. See the "useradd" man page for more information. Determining the inactivity timeout must be done with careful consideration of the length of a "normal" period of inactivity for users in the particular environment. Setting the timeout too low incurs support costs and also has the potential to impact availability of the system to legitimate users.'
  end
